...
public:
	AppFrame() :
		wxFrame(NULL, wxID_ANY, "ZuluSCSI-V6-util", wxPoint(50, 50), wxSize(600, 700)),
		...
		myConsoleProcess(NULL)
	{
		...
		myLogWindow = new wxLogWindow(this, _("ZuluSCSI-V6-util debug log"), true);
...
private:
	...
	void doDFUUpdate(const std::string& filename)
	{
		if (filename.find(".dfu") == std::string::npos)
		{
			wxMessageBox(
				"Wrong filename",
				"ZuluSCSI V6 requires a .dfu file",
				wxOK | wxICON_ERROR);
			return;
		}
	...
	void OnID_Timer(wxTimerEvent& event)
	{
		...
		try
		{
			...
			if (!myHID)
			{
				...
				{
					std::stringstream msg;
					msg << "ZuluSCSI V6 Ready, firmware version " <<
...
*/
				...
				else
				{
					char ticks[] = {'/', '-', '\\', '|'};
					std::stringstream ss;
					ss << "Searching for ZuluSCSI V6 device " << ticks[myTickCounter % sizeof(ticks)];
					myTickCounter++;
	...
	{
		wxMessageBox(
			"ZuluSCSI-V6-util Copyright (C) 2024 Rabbit Hole Computing LLC\n"
...
"along with this program.  If not, see <http://www.gnu.org/licenses/>.\n",
			...
			"About ZuluSCSI-V6-util", wxOK | wxICON_INFORMATION );
	}
